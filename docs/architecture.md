...
- **Prevents:** floating-point drift producing a total that is one fil off,
  on the exact screen whose credibility is the entire product.
- **Rule:** every monetary value is an integer count of fils
  (`45.50 AED → 4550`) in the database, in transit, and in domain code.
...
| Money | Integer fils everywhere; formatted only by `lib/domain/money.ts` (AD-6) |
...
- `orders` — `order_ref`, `fulfilment_mode`, `table_number`, `address`,
  `diner_phone`, `note`, `status`, `total_fils`, `assigned_driver_id`,
  `flagged_reason`.
...
- **Online card payment** — out of MVP scope (PRD §6.2). Deferred deliberately:
  `orders` already carries `total_fils`, so adding a payment provider later
